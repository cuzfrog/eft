...
/**
  * Stream tcp utility.
  */
private class DualPortTcpMan(systemName: String = "eft",
  ...
  def pull(codeInfo: RemoteInfo, folder: Path): Unit = {
    ...
    val remoteInfo = sendCmd(remoteIP, codeInfo.port, Ask) match {
    ...
    receiveServer.runForeach { connection =>
      ...
      ioResultF.map {
        case iOResult if iOResult.wasSuccessful =>
          debug("File pulled.")
          sendCmd(remoteIP, codeInfo.port, Done)
          system.terminate()
          iOResult
        case failed =>
    ...
    sendCmd(remoteIP, codeInfo.port, pullInfo)
